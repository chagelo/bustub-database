...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw ExecutionException(fmt::format("the number of arguments should be 1."));
  }

  if (args[0]->GetReturnType() != TypeId::VARCHAR) {
    throw Exception(ExceptionType::MISMATCH_TYPE, fmt::format("func {} expect type VARCHAR", func_name));
  }

  StringExpressionType str_exper_type =
      func_name == "lower" ? StringExpressionType::Lower : StringExpressionType::Upper;
  AbstractExpressionRef temp = std::make_shared<StringExpression>(args[0], str_exper_type);
  return temp;
}
